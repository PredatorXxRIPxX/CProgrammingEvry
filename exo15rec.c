...
void bruteforce(int first, int step, int zero) {
    ...
    sprintf(filename, "found.PID.%d", getpid());
    ...
    // Loop termination condition (example: maximum of 1000 iterations)
    ...
    while (1) { // Replace with a more defined loop condition
        sprintf(str, "%d", first);
        md5hash(str);
        if (zeros(hash, zero)) {
            break;
